...
static void load_digit_image_into_slot(int slot_number, int digit_value) {
  ...
  GRect frame = (GRect) {
    .origin = { (slot_number % 2) * 72, (slot_number / 2) * 72 },
...
static void init() {
  ...
  //app_log(APP_LOG_LEVEL_DEBUG, "big_time.c", 194, "frame size=(%d, %d)", frame.size.w, frame.size.h);

                                                //   x,  y                w,          h
  dayLayer = text_layer_create((GRect) { .origin = { 0, frame.size.h-20 -DATE_OFFSET}, .size = { frame.size.w-DATE_OFFSET, 20 } });

  GSize size = graphics_text_layout_get_content_size("42", font, (GRect) { .origin = { 0, 0 }, .size = { 100, 100 } }, GTextOverflowModeTrailingEllipsis, GTextAlignmentLeft);
  app_log(APP_LOG_LEVEL_DEBUG, "big_time.c", 202, "text size=(%d, %d)", size.w, size.h);

  //dayLayer = text_layer_create(GRect(0, 20, frame.size.w, 20));
  //dayLayer = text_layer_create(GRect(0, 20, frame.size.w, frame.size.h - 20));
  ...
  //text_layer_set_overflow_mode(dayLayer, GTextOverflowModeTrailingEllipsis);
  //layer_set_clips((Layer *)dayLayer, false);
  layer_add_child(root_layer, text_layer_get_layer(dayLayer));  
